...
- **`herm skills` — a skill package manager.** A herm-owned `skillpm` engine (`~/.hermes/skillpm/`) reconciles the live skill set from a lockfile (`~/.hermes/skill-lock.toml`). `herm skills deploy|list|sync|enable|disable` self-deploys the engine + catalog over `tailscale ssh` (no root, no reprovision) and reloads the gateway gracefully on mutations. First run seeds the catalog **and** preserves any skills already on the VM. `cloud-init/scripts/07-seed-skills.sh` installs the engine into `~/.hermes` and reconciles at boot (replaces the old blind `rsync`).
